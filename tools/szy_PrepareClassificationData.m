function [tr_dat, tt_dat, tr_lb, tt_lb] = szy_PrepareClassificationData(...
    AllSamples, classNum, sampleNumInClass, trainingNum)
% [tr_dat, tt_dat, tr_lb, tt_lb] = szy_PrepareClassificationData(AllSamples, classNum, sampleNumInClass, trainingNum)
% 准备用来进行分类的样本数据。
% AllSamples是一个矩阵，表示所有样本，每一列是一个样本，classNum表示总共有多少类，
% sampleNumInClass表示每类有多少个样本，trainingNum表示每类中拿前多少个作为训练样本，
% 其余作为测试样本。trainSamples和testSamples都是矩阵，每一列是一个样本，分别表示训练样本和测试样本。
...
%所有训练样本
tr_dat = AllSamples(:,tr_index);
%所有测试样本
tt_dat = AllSamples(:, tt_index);
