function [tr_HighLevelFeature, tt_HighLevelFeature] = szy_DBNExtractHighLevelFeature(tr_dat, tr_lb, tt_dat, structure, batch_size, num_epoch)
...
rng(0);
sae = saesetup(structure); % //其实这里nn中的W已经被随机初始化过
...
% Train the FFNN
% nn.testing = 1;
nn = nnff(nn, tr_dat, zeros(size(tr_dat,1), nn.size(end)));
tr_HighLevelFeature = nn.a{end};
tr_HighLevelFeature = tr_HighLevelFeature';

nn = nnff(nn, tt_dat, zeros(size(tt_dat,1), nn.size(end)));
tt_HighLevelFeature = nn.a{end};
tt_HighLevelFeature = tt_HighLevelFeature';
